...
1. **初始化**：启动构建，加载配置文件
2. **构建依赖树**：从 entry 开始解析代码文件，查询模块间的依赖关系，构建依赖树
3. **模块转换**：遍历依赖树进行解析，调用所配置的 loader 和规则对模块进行转换
4. **模块打包**：根据依赖关系，将模块合并到一个或多个输出文件中，这步是可以修改输出内容的最后机会，可进行优化操作操作
5. **输出文件**：根据配置确定输出的路径和文件名，把输出文件写入到文件系统
